// routes/posts.js — Feed, create (with image/video), like, comment, delete
...
// ── Multer setup — saves uploaded files to backend/uploads/ ───
...
router.post('/', requireAuth, upload.single('media'), async (req, res) => {
  ...
  // If a file was uploaded, build its public URL
  let media_url  = '';
  let media_type = '';
  if (req.file) {
    media_url  = `http://localhost:${process.env.PORT || 5000}/uploads/${req.file.filename}`;
...
router.post('/:id/comments', requireAuth, async (req, res) => {
  ...
  if (content.length > 300)       return res.status(400).json({ error: 'Comment cannot exceed 300 characters.' });
